<template>
    <scheduler-frame  @submitData="submit" :method="props.method" :ResourceType="props.ResourceType">
        <template #strategy>
            ...
            <el-col :span="6">
                <el-form-item label="可超出最大值" prop="" v-if="item.spec.updateStrategy.type=='RollingUpdate'"  >
                    <StringOrNumber v-model:data="item.spec.updateStrategy.rollingUpdate.maxSurge"></StringOrNumber>
                </el-form-item>
            </el-col>
        </template>
    </scheduler-frame>
