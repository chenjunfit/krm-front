        ...
        <el-tabs v-model="ActiveName" class="demo-tabs">
            <el-tab-pane label="基本配置" name="basic" style="height: 180px">
                <el-row :gutter="20">
                    ...
                    <el-col :span="9">
                        <el-form-item label="Secret" prop="secret.secretName" >
                            <div class="flex flex-wrap gap-4 items-center">
                                <el-select
                                        v-model="newVolumeConfig.secret.secretName"
                    ...
                    <el-col :span="5">
                        <el-form-item label="key必须存在" title="当指定item时key是否必须存在" prop="newVolumeConfig.secret.optional" >
                            <el-switch v-model="newVolumeConfig.secret.optional"></el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="5" v-if="props.method=='create'">
                        <el-form-item  label="权限" prop="defaultMode"  >
                            <Oct2Decimal  v-model:data="newVolumeConfig.secret.defaultMode" placeholder="请输入权限"></Oct2Decimal>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-tab-pane>
            <el-tab-pane label="自定义挂载路径" name="mount" style="height: 180px">
                <el-table :data="newVolumeConfig.secret.items" >
                    <el-table-column label="Key" align="center" width="180" >
                        <template #default="scope">
                            <el-form-item :prop="('secret.items.'+scope.$index+'.key')" required :rules="rules.items.key">
                    ...
                    <el-table-column label="Path"  align="center" width="180">
                        <template #default="scope">
                            <el-form-item :prop="('secret.items.'+scope.$index+'.key')" required :rules="rules.items.path">
                    ...
                    <el-table-column label="添加"  align="center">
                        ...
                        <template #default="scope">
                            <el-button type="warning" @click="deleteTableRow(newVolumeConfig.secret.items,scope.row)" color="red">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
...
const data=reactive({
    ...
    rules:{
        ...
        'secret.secretName':[
            {required:true,message:"请选择secret",trigger:'blur'}
        ],
        items:{
...
const addItem=()=>{
    ...
    data.newVolumeConfig.secret.items.push(item)
}
</script>
